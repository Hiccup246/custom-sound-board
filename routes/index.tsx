...
export async function soundClipPaths(
  currentPath: string,
): Promise<SoundClip[]> {
  ...
  for await (const dirEntry of Deno.readDir(currentPath)) {
    ...
    if (dirEntry.isFile && dirEntryName.endsWith(".mp3")) {
      ...
      const clipPathWithoutStaticDir = "." +
        entryPath.slice(endOfFirstDirectory);

      soundClips.push({
        src: clipPathWithoutStaticDir,
        name: clipNameWithoutExtension,
      });
    }
...
export async function soundClipDirectories(
  currentPath: string,
): Promise<string[]> {
  ...
  return (
    <div class="flex flex-col">
      ...
      <SoundBoard
        ...
        soundClipDirs={data.soundClipDirs}
      />
      <SoundBoardFooter />
    </div>
  );
